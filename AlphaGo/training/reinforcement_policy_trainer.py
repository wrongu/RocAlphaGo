...
def make_training_pairs(player, opp, features, mini_batch_size, board_size=19):
	...
	# First we want to prep the states so that half of the boards get a move.
	# The other half we want to leave alone so that the second player makes the first move (being black).
	# Decided to alternate every board because this is how humans would play a match.
	def play_half_of_boards(states, moves, X_list, y_list, player_color):
		for st, mv, X, y, should_move in zip(states, moves, X_list, y_list, itertools.cycle([True, False])):
			if should_move:
				if st.current_player == player_color:
	...
	states, X_list, y_list = play_half_of_boards(states, moves_player1, X_list, y_list, player_color)
